...
- **FileAdapter port** — a read-only Python `Protocol` (six methods; all but
  `load_project` take a resolved `Project`) that the backend depends on via
  `Depends()`. Two implementations:
