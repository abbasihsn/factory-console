...
>
> **Addendum (T80 amendment 3) — the resolution invariant.** A run-state resolution that **could not read** something it needed must **refuse**; it may never fall back to a state that is *more permissive* than the one it failed to check. "I looked and found nothing" and "I could not look" are different answers, and only the first may return a mutable state. Concretely for the marker directory: the precedence walk (`merged` > `ready` > `in-flight` > `todo`) returns a marker only when every state **at or above** it was readable, so a stale `todo/<id>` under an unreadable `merged/` resolves `unreadable`, not the mutable `todo`. A state directory **below** an answer already read does not change it. The same rule binds source **discovery**: a location that could not be probed becomes the source and refuses, rather than being skipped into "this project has no run-state".
