...
>
> 1. **An id NAMED under such a subdirectory resolves `unreadable`**, refused by both gates. **Per id, never per source** — an unrecognised directory that names nobody changes nothing, because a misplaced folder must not turn a whole project read-only.
