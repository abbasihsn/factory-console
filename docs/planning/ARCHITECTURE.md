...
- **Dockerfile**: multi-stage (node builder → python builder → thin runtime) producing a lean, self-contained image. Not the primary distribution; uses floating major-version base tags (patches over bit-for-bit reproducibility). **Registry-less by decision (v3.0)**: no `VOLUME` and no `FACTORY_CONSOLE_DB_PATH`, because the image serves the one project bind-mounted at `/project` for the life of one `docker run` and has no multi-project serve mode to persist a registry for. `HOME` is pinned to the fc-owned `/home/fc`, so the console-owned store is still created lazily and correctly-permissioned inside the container's writable layer and dies with the container; persisting it is the operator's call (mount uid-1000-writable storage at `/home/fc/.factory-console`).
- **CI**: matrix over `{ubuntu-latest, macos-latest} × {Python 3.11, 3.12}` — lint, pytest, Vitest, wheel build, smoke install, Playwright e2e. Concurrency group cancels stale runs. The `test` job exports `FACTORY_CONSOLE_DB_PATH` under `$RUNNER_TEMP` and ends with an `always()` step failing the job if `~/.factory-console/` exists on the runner — the CI half of the harness's own leak guard.
