...
**T78, T79 and T85 have no dependencies and share no files** — but *"any two can run
concurrently"*, which this section originally claimed, is **wrong**, and a dry-run said so:

```
factory: dry-run: sprint 4 for milestone v2.1: T78 T79 T85
factory: dry-run: track file-adapter: T78 T79
factory: dry-run: track infra-devops: T85
  … (max 3 concurrent, same track serialized)
```

**The factory serializes by `track`, not by file overlap.** T78 and T79 are both
`file-adapter`, so they queue behind each other however disjoint their files are. The
concurrent pairs are **T78 ∥ T85** and **T79 ∥ T85** — different tracks.

Recorded rather than quietly corrected, because the mistake is instructive: I reasoned
about the property I could see in the tickets (shared files) while the scheduler reasons
about a property declared in the manifest (`track`). Independence is whatever the
scheduler measures, not whatever is true.
