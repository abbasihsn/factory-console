...
- Types come from `src/lib/api/types.ts` (generated, `DO NOT EDIT`). Regenerate with **`make codegen` from the repo root**, not `pnpm codegen` from here: the Make target builds the schema in-process from the models on disk (`scripts/openapi_schema.py` → `frontend/.openapi.json`, gitignored) and then runs the pnpm script over it — `openapi-typescript` (`--immutable`, so every field is `readonly`), then `scripts/postcodegen.mjs` to prepend the DO-NOT-EDIT banner. `pnpm codegen` alone reads whatever `FC_OPENAPI_URL` points at, defaulting to that generated file.
  - **Why not fetch a running backend.** It used to default to `http://127.0.0.1:8000/api/v1/openapi.json` — whatever process was listening on that port. A dev server left running from an earlier checkout answers happily, `openapi-typescript` reports success, and `types.ts` is silently regenerated against models that no longer exist. That is exactly how it failed: a newly added response field was simply absent from a codegen that had just succeeded. Building the schema in-process makes a stale answer unrepresentable, and no port is bound doing it.
